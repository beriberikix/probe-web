//! CMSIS-SVD parser and peripheral variable cache for debugging in the worker: a copy of
//! `probe-rs serve`'s `rpc/svd.rs`, with value reads awaiting the fork's async `MemoryInterface`
//! and parsing from the uploaded file's bytes (the worker has no file system).
