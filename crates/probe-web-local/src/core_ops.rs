...
use probe_rs::{
    CoreDump, CoreStatus, Error, InstructionSet, RegisterId, RegisterValue, Session as PrSession,
...
use probe_rs_rpc::{
    ...
    core_ops::{
        CoreAccessRequest, CoreBreakpointsRequest, CoreDumpRequest, CoreReadRegistersRequest,
        CoreVectorCatchRequest, CoreWriteRegRequest, HandleSemihostingRequest,
        HandleSemihostingResponse, HandleSemihostingResult, StepRequest, StepResponse, StepResult,
        WireCoreDump, WireCoreMetadata, WireInstructionSet, WireRegisterId, WireRegisterReadResult,
        WireRegisterValue, WireSemihostingUiEvent, WireSteppingMode, WireVectorCatchCondition,
    },
...
pub async fn core_step(ctx: &mut Ctx, _h: VarHeader, req: StepRequest) -> StepResult {
    ...
    if matches!(req.mode, WireSteppingMode::StepInstruction) {
        ...
        return Ok(StepResponse {
            status: status(st),
            program_counter: info.pc,
            warning: None,
        });
    }
    let debug_info = debug_info
        .ok_or_else(|| err("statement stepping needs debug info (debug_state/load_debug_info)"))?;
...
pub async fn core_write_reg(ctx: &mut Ctx, _h: VarHeader, req: CoreWriteRegRequest) -> NoResponse {
    ...
    let session = inner.session(req.sessid)?;
    let mut core = session.core(req.core as usize).await.map_err(err)?;
    core.write_core_reg(RegisterId(req.id.0), register_value(req.value))
        .await
        .map_err(err)?;
    Ok(())
}
...
pub async fn core_enable_vc(
    ...
    let condition = match req.condition {
        ...
        WireVectorCatchCondition::Svc | WireVectorCatchCondition::Hlt => {
            return Err(err(
                "this vector catch condition is not supported by the WebUSB transport",
            ));
        }
    };
    let mut inner = ctx.inner.lock().await;
...
pub async fn core_metadata(
    ...
    Ok(WireCoreMetadata {
        ...
        instruction_set: instruction_set(set),
    })
}
...
pub async fn core_read_registers(
    ...
    for id in req.ids {
        let result = core
            .read_core_reg::<RegisterValue>(RegisterId(id.0))
            .await
            .map(wire_register_value)
            .map_err(err);
...
pub async fn cores_status(ctx: &mut Ctx, _h: VarHeader, req: CoresRequest) -> CoresStatusResponse {
    ...
    let mut statuses = Vec::new();
    for idx in core_indices(session, req.cores.as_deref()) {
        let result: Result<CoreStatus, Error> =
            async { session.core(idx).await?.status().await }.await;
        match result {
            Ok(st) => statuses.push((idx as u32, status(st))),
            Err(Error::CoreDisabled(_)) => {}
            Err(e) => return Err(err(e)),
...
pub async fn core_dump(
    ...
    let session = inner.session(req.sessid)?;
    let mut core = session.core(req.core as usize).await.map_err(err)?;
    let dump = CoreDump::dump_core(&mut core, req.ranges)
        .await
        .map_err(err)?;
    Ok(WireCoreDump {
        registers: dump
            .registers
            .into_iter()
            .map(|(id, v)| (WireRegisterId(id.0), wire_register_value(v)))
            .collect(),
        data: dump.data,
...
pub async fn handle_semihosting(
    ...
    let mut core = session.core(req.core as usize).await.map_err(err)?;
    let st = core.status().await.map_err(err)?;
    let CoreStatus::Halted(probe_rs::HaltReason::Breakpoint(
        probe_rs::BreakpointCause::Semihosting(command),
    )) = st
    else {
        ...
        crate::semihosting::Outcome::Continue(events) => {
            let events = events
                ...
                .map(|event| match event {
                    probe_rs_rpc::monitor::SemihostingEvent::Output { data, .. } => {
                        WireSemihostingUiEvent::RttOutput { handle: 0, data }
                    }
                })
                .collect();
            core.run().await.map_err(err)?;
            Ok(HandleSemihostingResult {
                status: status(CoreStatus::Running),
                events,
            })
        }
        crate::semihosting::Outcome::Exit(reason) => {
            let message = match reason {
                probe_rs_rpc::monitor::MonitorExitReason::SemihostingExit(Ok(())) => {
                    "Application has exited with success.".to_string()
                }
                probe_rs_rpc::monitor::MonitorExitReason::SemihostingExit(Err(e)) => {
                    format!(
                        "Application has exited with reason {:#x}, subcode {:?}.",
                        e.reason, e.subcode
                    )
                }
            ...
            Ok(HandleSemihostingResult {
                status: status(st),
                events: vec![WireSemihostingUiEvent::LogToConsole(message)],
            })
        }
    }
}
