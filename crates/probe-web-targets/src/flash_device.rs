//! The `FlashDevice` C struct that every CMSIS flash algorithm carries.
//!
//! Ported unchanged from `target-gen/src/flash_device.rs` on the fork
//! (`beriberikix/probe-rs@webusb/nusb-0.2.7`). It was already free of I/O — it reads
//! out of an ELF buffer we hand it — so nothing had to change for wasm.
...
impl FlashDevice {
    ...
    pub(crate) fn new(elf: &goblin::elf::Elf<'_>, buffer: &[u8], address: u32) -> Result<Self> {
        ...
        let data = crate::flm::read_elf_bin_data(elf, buffer, address, Self::INFO_SIZE)
    ...
    pub(crate) fn parse_sectors(
        ...
        while let Some(data) =
            crate::flm::read_elf_bin_data(elf, buffer, address + offset, Self::SECTOR_INFO_SIZE)
        {
